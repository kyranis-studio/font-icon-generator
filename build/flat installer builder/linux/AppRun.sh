...
if [ -z "$SKIP" ] ; then
  ...
  desktop-file-install --rebuild-mime-info-cache \
    --vendor=$VENDORPREFIX --set-key=Exec --set-value="\"${HOME}/.local/share/font icon generator/font-icon-generator.sh\" %U" \
    --set-key=X-AppImage-Comment --set-value="Convert svg to font" \
    --set-icon="$VENDORPREFIX-$APP"  "$DESKTOP_FILE" \
